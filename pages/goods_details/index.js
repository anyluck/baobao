...
Page({
  ...
  */
  ...
  onLoad: function (options) {
   ...
   // console.log(that.data.myfunc)
    ...
    that.setData({
      vip_id: that.vip_id,
      dprice: options.dprice,
      mytext:that.data.mytext
    })
  ...
  getGoodsDetails:function(){
    ...
    getProductDetail(that.data.id).then(res=>{
      ...
      console.log("detail", storeInfo)
      that.pid=storeInfo.id
      that.cateid = storeInfo.cate_id
      if (that.data.dprice != 0) {
        storeInfo.price = that.data.dprice
      }
  ...
  */
  ...
  goDistribution: function(){
    var data = {};
    var that=this
    if(that.data.mytext == '立即下架')
    {
      data.uid = wx.getStorageSync("uid");
      data.pid = that.pid;
      DropDistributionProduct(data).then(function (res) { })
      return;
    }
    console.log("分销价",inputValue)//用这个
    data.uid = wx.getStorageSync("uid");
    // that.pid = storeInfo.id
    // that.cateid = storeInfo.cate_id

    data.pid = that.pid;
    data.dprice = inputValue
    if (inputValue==""){
      wx.showToast({
        title: '请输入分销价',
      })
