...
Page({
  ...
  onShow: function () {
    ...
    if (type == 1) {
      ...
      this.setData({ 'parameter.title': '佣金记录', name: '佣金总计', recordType: 3 });
    } else {
      wx.showToast({
        title: '参数错误',
        icon: 'none',
  ...
  getRecordList: function () {
    ...
    spreadCommission(recordType,{page:page,limit:limit}).then(res=>{
      ...
      that.setData({ recordCount: res.data.total || 0, status: limit > len, page: limit + page, recordList: recordListNew });
    });
  },
  getRecordListCount:function(){
    var that = this;
    spreadCount(that.data.recordType).then(res=>{
      that.setData({ recordCount: res.data.total });
    });
  },
  /**
   * 生命周期函数--监听页面隐藏
